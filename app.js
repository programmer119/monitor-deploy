...
function renderServer(){
  ...
  $('#serverRunningCount').textContent=sum.service_running||0;$('#serverWarningCount').textContent=warningServices;$('#serverDownCount').textContent=sum.service_down||0;$('#serverReviewCount').textContent=reviewServices;
  const machineState=sum.status==='critical'?'critical':sum.status==='warning'?'warning':'normal';const hero=$('#serverHealthHero');hero.className=`server-health-hero ${machineState}`;$('#serverHealthState').textContent=machineState==='critical'?'즉시 조치 필요':machineState==='warning'?'주의 항목 있음':'서버 정상';$('#serverHealthDetail').textContent=`서비스 ${sum.service_running||0}/${sum.service_total||0} · ROOT ${pct(root.used_percent)} · RAM ${pct(mem.used_percent)} · PostgreSQL ${pg.connect_ok?'정상':'확인 필요'}`;
